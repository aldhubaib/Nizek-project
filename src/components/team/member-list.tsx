    ...
    <div className="space-y-1">
      {members.map((member) => {
        ...
        return (
          <div
            key={member.id}
            className="flex items-center justify-between rounded-lg bg-card border border-border px-4 py-3 hover:border-muted-foreground/20 transition-colors"
          >
            ...
            <div className="flex items-center gap-2">
              {isAdmin && !isSelf ? (
                <>
                  ...
                  >
                    <SelectTrigger className="h-7 w-[150px] text-[11px] rounded-full border-border">
        ...
        <>
          ...
          {invitations.map((inv) => {
            ...
            return (
              <div
                key={inv.id}
                className="flex items-center justify-between rounded-lg bg-card border border-dashed border-border px-4 py-3"
              >
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-full bg-muted/50 flex items-center justify-center text-[11px] font-semibold text-muted-foreground shrink-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[10px] text-muted-foreground/50">
                        Invited {new Date(inv.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isAdmin && (
                    <>
                      ...
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${resendingId === inv.id ? "animate-spin" : ""}`} strokeWidth={1.5} />
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon-sm"
