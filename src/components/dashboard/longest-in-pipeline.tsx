...
function CompactRow({ task, maxMs }: { task: PipelineTask; maxMs: number }) {
  ...
  return (
    <Link href={`/dashboard/projects/${task.project.id}/tasks/${task.id}`} target="_blank" className="flex items-center gap-3 px-4 py-2.5 hover:bg-accent/20 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className={cn("text-xs font-semibold px-1.5 py-0 rounded-full border shrink-0", STAGE_COLORS[task.stage] ?? "bg-muted text-muted-foreground border-border")}>
            {task.stageLabel}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-[260px]">
        {data.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-s text-muted-foreground">No stalled tasks</p>
          </div>
        ) : (
