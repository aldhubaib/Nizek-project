...
import { ClipboardCheck, X, ExternalLink, Sparkles, Zap, Bug, AlertCircle, Palette, Clock } from "lucide-react";
...
export function PmQueue({ data }: Props) {
    ...
    return (
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          ...
          <h2 className="text-[13px] font-semibold text-foreground">PM Review</h2>
        </div>
        <div className="px-4 py-8 text-center">
          <p className="text-[12px] text-muted-foreground">No tasks awaiting internal review.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setExpanded(false)}>
          <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
              <div className="flex items-center gap-2">
                <ClipboardCheck className="w-4 h-4 text-amber-400" />
                <h2 className="text-[14px] font-semibold text-foreground">PM Review — Internal Review</h2>
...
function TaskRow({ task, showProject = false }: { task: Task; showProject?: boolean }) {
  ...
  return (
    <Link href={`/dashboard/projects/${task.projectId}/tasks/${task.id}`} target="_blank" className="flex items-center gap-3 px-4 py-2.5 hover:bg-accent/20 transition-colors group">
