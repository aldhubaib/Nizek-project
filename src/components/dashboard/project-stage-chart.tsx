    ...
    <div className={cn("app-card relative rounded-xl border border-border bg-card p-4", className ?? "mt-4")}>
      ...
      <div className="flex items-start justify-between gap-2 mb-4">
        <div className="flex items-center gap-s">
          ...
          <div>
            <h2 className="text-s font-semibold leading-tight">Tasks by stage</h2>
          ...
          {data.projects.map((project) => (
            <div key={project.id} className="flex flex-col items-center gap-2 flex-1 min-w-0">
              ...
              <div className="relative group flex flex-col items-center gap-0.5">
                <span className="w-6 h-6 rounded-full bg-muted border border-border text-xs font-bold text-muted-foreground flex items-center justify-center">
                  {project.name.trim().slice(0, 1).toUpperCase()}
                </span>
                <span className="text-xs text-muted-foreground tabular-nums">{project.total}</span>
                <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 z-10 whitespace-nowrap rounded-md border border-border bg-popover px-2 py-1 text-xs font-medium text-foreground shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                  {project.name.trim()}
                </span>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground/60 truncate">{stages}</span>
                      <span className="ms-auto text-s font-bold tabular-nums">{count}</span>
                      <span className="text-xs text-muted-foreground tabular-nums w-9 text-end">{pct}%</span>
                    </div>
