...
/** The id the board gives the bucket for contacts that are in no column. */
...
/** Every column accepts every contact, so there is no invalid-target state. */
...
interface ColumnProps extends Omit<CardListProps, "emptyLabel"> {
  ...
  onDelete: (stage: ContactStageDTO) => void;
  /** Dragging a column is off while a filter hides some of its cards. */
  reorderable: boolean;
}

export function ContactStageColumn({
  stage,
  contacts,
  busyId,
  onOpenContact,
  ...
  const {
    ...
    id: stage.id,
    data: { type: "column" },
    // The column still has to accept card drops while it cannot be picked up
    // itself, so only the draggable half is switched off.
  ...
  return (
    <div
      ...
      className={cn(
        COLUMN_SHELL,
        isOver && OVER_SHELL,
        // Lifted rather than hidden: a column is the width of the gap it leaves,
        // so fading it in place reads better than collapsing the strip.
...
/**
 * The bucket for contacts in no column — what a deleted column leaves behind.
 *
 * A plain droppable, not a sortable: it has no row of its own to reorder, and
 * the board only renders it while it holds somebody.
 */
...
function StageEditor({
  ...
  return (
    <div className="space-y-2.5">
      ...
      {/* The same swatches the project boards use for their columns, so a
