...
export function ImpersonationBanner({ targetName }: { targetName: string }) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[10000] flex items-center gap-3 rounded-full border border-amber-500/40 bg-amber-500/15 px-4 py-2 shadow-xl backdrop-blur-md">
      ...
      <span className="text-[12px] text-amber-100 whitespace-nowrap">
      ...
      <button
        ...
        onClick={handleExit}
        disabled={leaving}
        className="flex items-center gap-1 rounded-full bg-amber-500/25 hover:bg-amber-500/40 px-2.5 py-1 text-[11px] font-semibold text-amber-100 transition-colors disabled:opacity-50 whitespace-nowrap"
      >
