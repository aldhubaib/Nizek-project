...
import {
  getContactsPermissionAdminData,
  setUserContactsAccess,
  type ContactsMember,
...
export function ContactsAccessManager() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const data = await getContactsPermissionAdminData();
      if (cancelled) return;
  ...
  const toggle = (userId: string) => {
    ...
    startSaving(async () => {
      await setUserContactsAccess(userId, next);
    });
  };
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border/50 px-4 py-2 text-xs text-muted-foreground">
            ...
            <span>{allowed.size} with contacts access</span>
          </div>
