...
import { X, Loader2, MessageCircleQuestion, History, MessageSquare, ChevronRight, ChevronDown, Pencil, Check, Clock, Undo2, Gauge, Timer, FileText, Plus, Maximize2, Trash2, UserCircle2 } from "lucide-react";
...
export function TaskSidebar({ task, open, onClose, questions: allQuestions, projectId }: Props) {
  ...
  async function handleDecline() {
    ...
    setDeclining(true);
    try {
      await declineTask({ taskId: task.id, comment: declineComment.trim() });
                  ...
                  <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => { setShowDecline(false); setDeclineComment(""); }}
                        className="h-7 text-[11px]"
                      >
                        Cancel
                      </Button>
